...
- Bhadial/The Dial-compatible Conditions Score:
  ...
  percentile semantics, and payload fields are unchanged. Rolling median/MAD
  pressure factors use ordered sliding windows, and realized-volatility factors
...
- Generated `data/dashboard.json` from public data sources for HTTP/static
  ...
  fallback in `app.js`.
- Local SQLite history persistence in `data/history.sqlite3`, storing normalized
  key metrics for the full history while retaining the newest 30 full payloads
  by default. Full payloads use zlib compression; older rows keep metadata and
  metrics but discard the large body. Configure this through
  `MARCO_HISTORY_PAYLOAD_RETAIN`, `MARCO_HISTORY_PAYLOAD_COMPRESSION`, and
...
- Section 09 investment-view cards generated by deterministic, scenario-aware
  ...
  the ranked default view exposes priority, direction, sizing, trigger, and
  invalidation before the full thesis. Thesis text remains editable in-browser
  and can be replaced through `content/overrides.json`. Expanded rows also
  display a compact confidence overlay from `conclusionAudit` plus a historical
  S&P 500 price-index proxy for SPY impact based on matching Conditions Score
...
- Treasury scorecard assembly, percentile score thresholds, and compatibility
  ...
  both remain re-exported by `build_dashboard.py` so existing scripts keep the
  same import contract without making domain modules depend on the facade.
...
- Global LPPL Risk: an independent 0-100 global equity-index bubble/critical
  ...
  summary, and canonical `perIndexHistory`/`perIndexBacktests` maps. Index rows
  carry `historyRef`/`backtestRef` rather than duplicate those large blocks.
  The top-level
...
- Action-first disclosure controls remain keyboard reachable: secondary section
  headings support Enter/Space with synchronized `aria-expanded`, while native
  detail summaries, buttons, fields, and editable theses share a visible focus
  treatment across desktop and compact layouts.
...
dashboard file, and a cross-process lock prevents full and partial jobs from
racing. Independent public-source fetches use a bounded six-worker pool;
requests sharing one provider lane remain serialized and output ordering stays
deterministic. If a refresh returns real-source `error` rows while the current
...
- The headline Conditions Score uses the same public-source percentile
  machinery across 21 active factor definitions and 7 bhadial modules. High funding,
  credit, volatility, dollar, and energy stress are inverse-scored or
  shock-scored when high values indicate a less supportive backdrop. Internal
  factor reliability metadata keeps the existing numeric payload stable while
  preventing missing, stale, or insufficiently warmed inputs from being scored
  or labeled as real zero/normal observations. Components expose `observed`,
...
- The Global LPPL Risk indicator is a separate research diagnostic under
  ...
  metadata; each available index references its own canonical history curve and
...
- TreasuryDirect auctioned and announced securities JSON, including
  bid-to-cover percentile scoring against the same original security term and
  security type, so Bill, Note, and Bond demand regimes are not mixed. If the
...
- Stooq public quote CSV for continuous 30-Day Fed Funds futures `ZQ.F`, used
  only as one directional input for the qualitative Fed-path scenario. A
  continuous monthly-average contract is not treated as a source of
  meeting-level hike/hold/cut probabilities.
...
- Fed path probabilities fail closed: `fedPath` remains empty until
  meeting-specific Fed Funds or OIS contracts, a documented target-range
  mapping, and calibration evidence are supplied. `fedPathAudit` separately
  lists official future meetings at the dashboard `asOf` date and exposes a
  qualitative `modeled-scenario-only` direction from `ZQ.F`, the 2Y move, and
  tracked inflation. It explicitly sets `probabilitiesAvailable: false` and
  `isProbability: false`.
- The SEP dot plot field is official but low frequency; if the Federal Reserve
  ...
  than blocking the daily refresh. The curve/futures view remains a
  directional scenario only until meeting-specific market data is supplied.
...
The dashboard envelope is versioned by `schemaVersion` (`1.0.0`) and documented
in `schema/dashboard-v1.schema.json`. Dashboard, health, and history JSON GETs
emit SHA-256 `ETag` values and honor `If-None-Match` only for successful
responses. The frontend revalidates stable URLs with `cache: "no-cache"`;
original 4xx/5xx statuses and bodies are never rewritten as 304.
