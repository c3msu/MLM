...
| REST API | Done | Local Python server exposes dashboard slices for curve, decomposition, Fed path, scorecard, policy, auctions, positioning, cross-market, events, news, ideas, and source status. It supports ISO date-range filters on dated list endpoints and `format=csv` exports. |
...
| Daily background update | Done | `scripts/serve.py --daily-at HH:MM` and macOS LaunchAgent installer. Startup refresh and manual `POST /api/update` refreshes run in the background, data-file writes are atomic, and failed real-source refresh candidates no longer overwrite the last healthy dashboard. |
| Local monitoring | Done | `/api/health` reports update timestamps, source-status counts, real error sources, and latest history-backfill status. `scripts/check_health.py` and `scripts/install_health_agent.py` provide a daily local health check and optional macOS notification after the update window. |
